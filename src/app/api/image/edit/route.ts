...
function formatGeminiError(err: unknown): string {
  ...
  if (msg.includes("429") || msg.includes("RESOURCE_EXHAUSTED") || msg.includes("quota"))
    return "무료 사용량 한도를 초과했습니다. 내일 다시 시도하거나 유료 플랜으로 업그레이드하세요.";
  if (msg.includes("SAFETY") || msg.includes("PROHIBITED"))
    return "요청한 이미지가 Google 콘텐츠 정책에 위반됩니다. 지시를 수정해주세요.";
  if (msg.includes("403") || msg.includes("not supported"))
  ...
  try {
    ...
    if (!imagePart) {
      return NextResponse.json(
        { error: "이미지를 수정하지 못했습니다. 지시 내용을 수정해보세요." },
        { status: 500 }
      );
    }
